...
const albumCollection = [
...

    {
        ...
        artist: 'Sleep Token',
        tracks: ['Chokehold', 'Euclid', 'Granite']
    },

    {
        ...
        artist: 'Hozier',
        tracks: ['Almost', 'No Plan', 'Nina Cried Power']
    }
...
function updateAlbum(id, prop, value) {
    let index = albumCollection.findIndex(album=>album.id === id);

    if (index !== -1) {
        let album = albumCollection[index];

        if (!album.hasOwnProperty(prop) && value !== '') {
            album[prop] = value;
        }
        ...
        else if (album.hasOwnProperty(prop) && value === '') {
            delete album[prop];
        }
    }
